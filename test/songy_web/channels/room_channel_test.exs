defmodule SongyWeb.RoomChannelTest do
  ...
  setup do
    ...
    {:ok, pid} = GameSession.lookup_game_session(game.id)
    ...
    Repatch.patch(Songy.Boundary.Player, :search_random_track, [mode: :shared], fn _provider ->
      ...
      {:ok,
       %Track{
         ...
         year: 2023,
         meta: %{uri: "spotify:track:track-#{id}"}
       }}
    end)
  ...
  describe "join event" do
    test "broadcasts update with new participant", %{current_user: current_user, game: game} do
    ...
    test "new participant join with a random track in timeline", %{
      current_user: current_user,
      game: game
    } do
      {:ok, _reply, _socket} = join_room_channel(current_user, game.id)

      assert_broadcast("state_updated", %{timelines: timelines})
      assert [%Track{} | _] = timelines[current_user.uuid]
    end
  end

  describe "start_game event" do
    setup %{owner: owner, game: game} do
      join_room_channel(owner, game.id)
      :ok
    end

    test "changes game status", %{current_user: current_user, game: game} do
  ...
  describe "make assumption event" do
    setup %{owner: owner, game: game} do
      previous_timeout = Application.fetch_env!(:songy, :challenging_phase_timeout)
      Application.put_env(:songy, :challenging_phase_timeout, :infinity)
    ...
    test "adds turn track to timeline", %{
      ...
      assert_broadcast(
        "state_updated",
        %{turn: %{assumptions: [%{position: 0, user_id: _}]}}
      )
    end
  end

  describe "reorder timeline event" do
    setup %{owner: owner, game: game} do
      previous_timeout = Application.fetch_env!(:songy, :challenging_phase_timeout)
      Application.put_env(:songy, :challenging_phase_timeout, :infinity)
      on_exit(fn -> Application.put_env(:songy, :challenging_phase_timeout, previous_timeout) end)
